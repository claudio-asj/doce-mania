...
export function Product({ name, img, price, description }: ProductProp) {
    ...
    const checkIfFavorite = useCallback((productName: string): boolean => {
    ...
    const atualizaFavorito = useCallback(() => {
    ...
    return (
        <div className="relative cursor-pointer border-slate-50 border-2 rounded-lg shadow-lg overflow-hidden min-h-52 transform transition-transform duration-300 hover:scale-110">
            ...
            <button
                onClick={handleClick}
                className={`absolute top-3 right-2 rounded-full bg-slate-50/55 p-1 transform transition-transform duration-300 hover:scale-105 ${
