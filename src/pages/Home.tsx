      ...
      <main className="max-w-5xl mx-auto mt-16 px-4">
        <nav className="flex items-center justify-between gap-4 max-w-full overflow-x-scroll pb-4">
          <CategoryBtn
            title="Bolo"
            Icon={Cake}
            isSelected={categorySelected == "Bolo"}
            onClick={() => {
              changeCategorySelected("Bolo");
            }}
          />
          <CategoryBtn
            title="Salgado"
            Icon={Croissant}
            isSelected={categorySelected == "Salgado"}
            onClick={() => {
              changeCategorySelected("Salgado");
            }}
          />
          <CategoryBtn
            title="Sorvete"
            Icon={IceCream}
            isSelected={categorySelected == "Sorvete"}
            onClick={() => {
              changeCategorySelected("Sorvete");
            }}
          />
          <CategoryBtn
            title="Bebida"
            Icon={CupSoda}
            isSelected={categorySelected == "Bebida"}
            onClick={() => {
              changeCategorySelected("Bebida");
            }}
          />
          <CategoryBtn
            title="Doce"
            Icon={CakeSlice}
            isSelected={categorySelected == "Doce"}
            onClick={() => {
              changeCategorySelected("Doce");
            }}
          />
          <CategoryBtn
            title="Kit festa"
            Icon={PartyPopper}
            isSelected={categorySelected == "Kit festa"}
            onClick={() => {
              changeCategorySelected("Kit festa");
            }}
          />
        </nav>

        <h2 className="mt-16 text-2xl font-bold text-azulEscuro">
          {categoryTitle}
        </h2>
        {products.length > 0 ? (
